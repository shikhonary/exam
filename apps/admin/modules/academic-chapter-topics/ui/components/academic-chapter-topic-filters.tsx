...
import {
  useAcademicChapterTopicFilters,
...
export const AcademicChapterTopicFilters = ({ isLoading }: FilterProps) => {
  const [search, setSearch] = useState("");
  const [filters, setFilters] = useAcademicChapterTopicFilters();
  const debounceValue = useDebounce(search, 500);

  const { data: yearsData } = useAcademicYearsForSelection();
  const { data: subjectsData } = useAcademicSubjectsForSelection();
  const { data: chaptersData } = useAcademicChaptersForSelection(filters.subjectId);
  ...
  const selectedChapterLabel = chaptersData?.find(c => c.id === filters.chapterId)?.nameEn || "Chapter";
            ...
            <div className="flex flex-wrap items-center gap-2 px-4 pb-4 pt-0">
              ...
              {filters.chapterId && (
                <Badge
                  ...
                  className="flex items-center gap-1.5 px-3 py-1 bg-white border border-primary/10 text-xs text-primary shadow-sm rounded-lg hover:bg-white"
                >
                  <span className="font-bold text-[10px] uppercase opacity-50 mr-1 text-on-surface">Chapter:</span>
                  <span className="font-bold text-[11px]">{selectedChapterLabel}</span>
                  <button onClick={() => setFilters({ ...filters, chapterId: null })} className="hover:text-rose-500 transition-colors ml-1">
                    <X className="w-3 h-3 stroke-[3]" />
                  </button>
                </Badge>
              )}
