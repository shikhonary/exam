...
const columns: Column<AcademicChapterWithCount>[] = [
  {
    key: "position",
    header: "Order",
    render: (item) => (
    ...
    render: (item) => (
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center gap-1.5 text-[11px] font-bold text-foreground bg-muted/30 w-fit px-2 py-0.5 rounded border border-border/50">
          ...
          {item.subject?.nameEn || "No Subject"}
        </div>
        {item.academicYear && (
...
interface AcademicChapterTableProps {
  chapters: AcademicChapterWithCount[];
  isLoading: boolean;
  onDelete: (id: string, name: string) => void;
  onToggleActive: (id: string) => void;
}

export function AcademicChapterTable({
  chapters,
  isLoading,
  onDelete,
  onToggleActive,
}: AcademicChapterTableProps) {
    ...
    return (
      <div className="bg-card/40 backdrop-blur-xl rounded-3xl border border-dashed border-border/50 p-16 text-center shadow-soft m-4">
        ...
        <p className="text-sm text-muted-foreground mt-2 max-w-sm mx-auto">
          We couldn&apos;t find any academic chapters matching your current filters. Try
          broadening your search parameters or create a new chapter.
        </p>
      </div>
    );
  }
                  ...
                  <DropdownMenu>
                    ...
                    >
                      <DropdownMenuItem
                        ...
                        asChild
                      >
                        <Link href={`/academic-chapters/${item.id}/edit`}>
