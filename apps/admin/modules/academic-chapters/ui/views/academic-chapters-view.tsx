...
import {
  ...
  useToggleAcademicChapterActive,
  useAcademicChapterStats,
} from "@workspace/api-client";

import { AcademicChapterTable } from "../components/academic-chapter-table";
import { AcademicChapterFilters } from "../components/academic-chapter-filters";
import { AcademicChapterStats } from "../components/academic-chapter-stats";
...
function AcademicChaptersContent() {
  const { openDeleteModal } = useDeleteModal();
  const { data: chaptersData, isLoading: isQueryLoading } = useAcademicChapters();
  const { data: statsData, isLoading: isStatsLoading } = useAcademicChapterStats();

  const { mutateAsync: deleteChapter, isPending: isDeleting } = useDeleteAcademicChapter();
  const { mutateAsync: toggleActive, isPending: isToggling } = useToggleAcademicChapterActive();
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6">
        ...
        <div className="relative max-w-2xl animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <p className="text-sm leading-6 text-on-surface-variant max-w-lg font-medium italic opacity-70">
            Manage global academic chapters across the platform.
          </p>
        </div>
        ...
        <div className="flex-shrink-0 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-150">
          <Button
            ...
            "
          >
            <Link href="/academic-chapters/new">
      ...
      <div className="mb-8">
        <AcademicChapterStats stats={statsData} isLoading={isStatsLoading} />
      </div>
      ...
      <div className="bg-surface-container-lowest rounded-2xl shadow-ambient overflow-hidden flex flex-col border border-outline/5 relative isolate animate-in slide-in-from-bottom-8 duration-700 delay-200">
        <AcademicChapterFilters isLoading={isLoading} />
        
        <AcademicChapterTable
...
export function AcademicChaptersView() {
  return (
