...
export const AcademicSubjectFilters = ({ isLoading }: FilterProps) => {
  const [search, setSearch] = useState("");
  const [filters, setFilters] = useAcademicSubjectFilters();
  ...
  const selectedClassLabel = classesData?.find(c => c.id === filters.academicClassId)?.nameEn || "Class";
            ...
            <div className="flex flex-wrap items-center gap-2 px-4 pb-4 pt-0">
              ...
              {filters.academicClassId && (
                ...
                >
                  ...
                  <button onClick={() => setFilters({ ...filters, academicClassId: null })} className="hover:text-rose-500 transition-colors ml-1">
                    <X className="w-3 h-3 stroke-[3]" />
                  </button>
                </Badge>
              )}
              ...
              {currentSort && currentSort !== "nameEn-asc" && (
                <Badge
                  variant="secondary"
                  className="flex items-center gap-1.5 px-3 py-1 bg-white border border-primary/10 text-xs text-primary shadow-sm rounded-lg hover:bg-white"
                >
