...
export const CqImportView = () => {
  ...
  const { mutateAsync: bulkCreateCQs, isPending } = useBulkCreateCQs();
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      await bulkCreateCQs(previewData);
      router.push("/cqs");
    } catch (error) {
      toast.error("Failed to import CQs.");
      console.error(error);
    }
  };
  ...
  return (
    <div className="container mx-auto px-6 py-12 lg:px-12 max-w-7xl relative z-10 space-y-6">
      <div className="space-y-2">
        <h1 className="text-3xl font-bold">Import CQs</h1>
        <p className="text-muted-foreground">
          {previewData 
            ? `Previewing ${previewData.length} Creative Questions for import.` 
            : "Paste your JSON data below to preview and bulk import Creative Questions."}
        </p>
      </div>
      ...
      {!previewData ? (
        <div className="space-y-6 animate-in fade-in">
          ...
          <Card className="bg-card/80 backdrop-blur-2xl border-border/60 rounded-[2.5rem] overflow-hidden shadow-large relative transition-all duration-500 hover:shadow-glow/5">
            ...
            <CardHeader className="pb-4 px-8 pt-8">
              ...
              <CardDescription className="font-bold text-muted-foreground/80">
                Apply classification to all imported CQs
              </CardDescription>
            </CardHeader>
          ...
          <Textarea
            placeholder='[{"questionA": "What is...", "context": "...", ...}]'
            className="min-h-[400px] font-mono text-sm"
            value={jsonInput}
            onChange={(e) => setJsonInput(e.target.value)}
          />
          <div className="flex justify-end gap-4">
            <Button variant="outline" onClick={() => router.push("/cqs")}>
              Cancel
            </Button>
        ...
        <div className="space-y-6 animate-in slide-in-from-bottom-4">
          <div className="grid grid-cols-1 gap-4 p-2 border rounded-xl bg-muted/10">
            {previewData.map((cq, idx) => (
              <CqCard
                key={idx}
                cq={{ ...cq, id: `preview-${idx}` }}
                onUpdate={(updatedCq) => {
                  setPreviewData((prev) =>
                    prev ? prev.map((p, i) => (i === idx ? { ...p, ...updatedCq } : p)) : null
                  );
                  toast.success("CQ updated locally");
                }}
              />
            ))}
          </div>
