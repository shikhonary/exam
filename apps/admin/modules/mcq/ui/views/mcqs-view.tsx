...
import { McqCard, McqCardSkeleton, type McqCardItem } from "../components/mcq-card";
import { McqFilters } from "../components/mcq-filters";
...
export const McqsView = () => {
  ...
  const handleDelete = (id: string, questionText: string) => {
    openDeleteModal({
      entityId: id,
      entityType: "mcq",
      entityName: questionText ? (questionText.slice(0, 60) + (questionText.length > 60 ? "..." : "")) : "this MCQ",
      onConfirm: (id) => {
        deleteMCQ({ id });
      },
    });
    ...
    <div className="container mx-auto px-6 py-12 lg:px-12 max-w-7xl relative z-10 space-y-10 text-foreground animate-in fade-in duration-700">
      ...
      <div className="relative flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6">
        ...
        <div className="relative max-w-2xl animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <p className="text-sm leading-6 text-muted-foreground max-w-lg font-medium italic opacity-70">
            Manage multiple choice questions (MCQs) and bulk import them using JSON.
          </p>
        </div>
        ...
        <div className="flex-shrink-0 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-150 flex items-center gap-3">
          ...
          >
            <Link href="/mcqs/new">
              ...
              <span className="relative">Create New MCQ</span>
            </Link>
          </Button>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-10">
        ...
        <div className="space-y-6">
          <McqFilters isLoading={isLoading} />
          ...
          <div className="grid grid-cols-1 gap-4">
            {mcqs.map((mcq) => (
              <McqCard
                ...
                onUpdate={(updatedMcq) => {
                  updateMCQ({ id: mcq.id, data: updatedMcq }).then(() => {
