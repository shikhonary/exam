...
const columns: Column<Mcq>[] = [
  {
    ...
    render: (mcq) => (
      <div className="flex flex-col gap-1 max-w-sm">
        <p className="font-bold text-foreground tracking-tight text-sm line-clamp-2">
          {mcq.question}
        </p>
      </div>
    ),
  ...
  },
  {
    ...
    render: (mcq) => (
      <div className="flex flex-col gap-1">
        <p className="font-medium text-muted-foreground text-xs leading-none">
          {new Date(mcq.createdAt).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}
        </p>
      </div>
    ),
  },
