...
import { ShortAnswerCard } from "../components/short-answer-card";

export const ShortAnswerImportView = () => {
  const [jsonInput, setJsonInput] = useState("");
  const [previewData, setPreviewData] = useState<any[] | null>(null);
  const { mutateAsync: bulkCreateItems, isPending } = useBulkCreateShortAnswers();
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      await bulkCreateItems(previewData);
      router.push("/short-answers");
    } catch (error) {
      toast.error("Failed to import Short Answers.");
      console.error(error);
    }
  };
  ...
  return (
    <div className="container mx-auto px-6 py-12 lg:px-12 max-w-7xl relative z-10 space-y-6">
      <div className="space-y-2">
        <h1 className="text-3xl font-bold">Import Short Answers</h1>
        <p className="text-muted-foreground">
          {previewData 
            ? `Previewing ${previewData.length} Short Answers for import.` 
            : "Paste your JSON data below to preview and bulk import Short Answers."}
        </p>
      </div>
      ...
      {!previewData ? (
        <div className="space-y-6 animate-in fade-in">
          ...
          <Card className="bg-card/80 backdrop-blur-2xl border-border/60 rounded-[2.5rem] overflow-hidden shadow-large relative transition-all duration-500 hover:shadow-glow/5">
            ...
            <CardHeader className="pb-4 px-8 pt-8">
              ...
              <CardDescription className="font-bold text-muted-foreground/80">
                Apply classification to all imported Short Answers
              </CardDescription>
            </CardHeader>
          ...
          <Textarea
            placeholder='[{"question": "What is...", "answer": "...", "reference": []}]'
            className="min-h-[400px] font-mono text-sm"
            value={jsonInput}
            onChange={(e) => setJsonInput(e.target.value)}
          />
          <div className="flex justify-end gap-4">
            <Button variant="outline" onClick={() => router.push("/short-answers")}>
              Cancel
            </Button>
        ...
        <div className="space-y-6 animate-in slide-in-from-bottom-4">
          <div className="grid grid-cols-1 gap-4 p-2 border rounded-xl bg-muted/10">
            {previewData.map((item, idx) => (
              <ShortAnswerCard
                key={idx}
                item={{ ...item, id: `preview-${idx}` }}
                onUpdate={(updatedItem) => {
                  setPreviewData((prev) =>
                    prev ? prev.map((p, i) => (i === idx ? { ...p, ...updatedItem } : p)) : null
                  );
                  toast.success("Short Answer updated locally");
                }}
              />
            ))}
          </div>
