...
function SubscriptionPlansContent() {
  ...
  const { data: plansData, isLoading: isQueryLoading } = useSubscriptionPlans();
  ...
  const isLoading = isQueryLoading || isDeleting || isActivating || isDeactivating || isSettingPopular;
  ...
  const handleDelete = (id: string, label: string) => {
    openDeleteModal({
      ...
      entityName: label,
      onConfirm: (entityId) => {
        deletePlan({ id: entityId });
      },
    });
  };

  const handleToggleActive = (id: string) => {
    const plan = plansData?.items?.find((p: any) => p.id === id);
    if (!plan) return;
  ...
  const handleTogglePopular = (id: string, isPopular: boolean) => {
    setPopular({ id, isPopular });
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6">
        ...
        <div className="relative max-w-2xl animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <p className="text-sm leading-6 text-on-surface-variant max-w-lg font-medium italic opacity-70">
            Manage subscription plans, pricing tiers, and resource limits for tenants.
          </p>
        </div>
        ...
        <div className="flex-shrink-0 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-150">
          ...
          >
            <Link href="/subscription-plans/new">
              ...
              <span className="relative">New Plan</span>
            </Link>
          </Button>
        </div>
      </div>
      ...
      <div className="bg-surface-container-lowest rounded-2xl shadow-ambient overflow-hidden flex flex-col border border-outline/5 relative isolate animate-in slide-in-from-bottom-8 duration-700 delay-200">
        ...
        />
        <Pagination totalItem={plansData?.meta?.total ?? 0} />
      </div>
    </motion.main>
  );
}
