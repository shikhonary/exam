...
import { useUserFilters } from "@workspace/api-client";
...
export const Filter = ({ isLoading }: FilterProps) => {
  ...
  const handleFilterRoleChange = (value: string | null) => {
    setFilters({
      ...filters,
      role: value,
      page: 1,
        ...
        <div className="flex flex-wrap items-center gap-4 flex-1">
          ...
          <div className="w-full sm:w-40">
            <Select
              ...
              onValueChange={(val) =>
                handleFilterRoleChange(val === "all" ? null : val)
              }
              disabled={isLoading}
            >
            ...
            <div className="flex flex-wrap items-center gap-2 px-4 pb-4 pt-0">
              ...
              {filters.role !== null && (
                <Badge
                  ...
                  className="flex items-center gap-1.5 px-3 py-1 bg-white border border-primary/10 text-xs text-primary shadow-sm rounded-lg hover:bg-white"
                >
                  <span className="font-bold text-[10px] uppercase opacity-50 mr-1 text-on-surface">Role:</span>
                  <span className="font-bold text-[11px]">{filters.role}</span>
                  <button onClick={() => setFilters({ ...filters, role: null })} className="hover:text-rose-500 transition-colors ml-1">
                    <X className="w-3 h-3 stroke-[3]" />
                  </button>
                </Badge>
              )}
